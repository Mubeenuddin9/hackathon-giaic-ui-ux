      ...
      <nav className="flex items-center mt-6 mb-6">
        ...
        <div className="absolute sm:absolute md:static bg-white md:min-h-fit min-h-[60vh] left-0 top-[-100%] md:w-auto w-full fkex md:items-center px-5">
          <ul className="flex ml-[120px] space-x-5 text-gray-500 font-semibold items-center">
            <li className="hover:underline duration-500 active:underline">
              <Link href="/">Home</Link>
            </li>
            <li className="hover:underline duration-500 active:underline">
        ...
        <div className="flex items-center gap-6">
          ...
          <HiMenuAlt3
            className="absolute sm:absolute md:hidden text-5xl cursor-pointer ml-[130px]"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
          />

          <div
            className={`absolute xl:hidden top-24 left-0 w-full bg-white flex flex-col items-center gap-6 font-semibold text-lg
              transform transition-transform ${isMenuOpen ? "opacity-100" : "opacity-0"}`}
            style={{ transition: "transform 0.3s ease, opacity 0.3s ease" }}
          >
            <li className="list-none w-full text-center p-4 hover:bg-sky-400 text-black transition-all cursor-pointer">
              <Link href="/">Home</Link>
            </li>
            <li className="list-none w-full text-center p-4 hover:bg-sky-400 text-black transition-all cursor-pointer">
              <Link href="/components/shop">Shop</Link>
        ...
        <button className="hidden md:w-[252px] md:h-[48px] text-white font-semibold bg-sky-500 rounded-sm ml-10 md:flex items-center justify-center">
          Become a member
          <IoIosArrowRoundForward className="ml-2 w-7 h-7" />
        </button>
      </nav>
    </main>
  );
